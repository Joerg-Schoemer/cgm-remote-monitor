...
function create (env, ctx) {
  const app = express();
  const appInfo = env.name + ' ' + env.version;
  ...
  const insecureUseHttp = env.insecureUseHttp;
  const secureHstsHeader = env.secureHstsHeader;
  if (!insecureUseHttp) {
    ...
    if (secureHstsHeader) { // Add HSTS (HTTP Strict Transport Security) header
      ...
      if (enableCSP) {
        const secureCspReportOnly = env.secureCspReportOnly;
      ...
      const includeSubDomainsValue = env.secureHstsHeaderIncludeSubdomains;
      const preloadValue = env.secureHstsHeaderPreload;
      app.use(helmet({
        hsts: {
          maxAge: 31536000
  ...
  const staticFiles = express.static(resolvePath(env.static_files), {
    maxAge
  });
  ...
  if (env.settings.isEnabled('cors')) {
    const allowOrigin = _get(env, 'extendedSettings.cors.allowOrigin') || '*';
  ...
  const api = require('../api/')(env, ctx);
  const api2 = require('../api2/')(env, ctx, api);
  const api3 = require('../api3/')(env, ctx);
