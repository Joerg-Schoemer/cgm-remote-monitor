...
function init (ctx) {
  ...
  weektoweek.html = function html (client) {
    ...
    return `<h2>${translate('Week to week')}</h2>
<b>${translate('To see this report, press SHOW while in this view')}</b><br>
&nbsp;${translate('Size')} 
<select id="wrp_size"> 
    <option data-x="800"  data-y="250">800x250px</option>
    <option data-x="1000" data-y="300" selected>1000x300px</option>
    <option data-x="1200" data-y="400">1200x400px</option>
    <option data-x="1550" data-y="600">1550x600px</option>
</select><br>
${translate('Scale')}: 
<input type="radio" name="wrp_scale" id="wrp_linear" checked>${translate('Linear')}
<input type="radio" name="wrp_scale" id="wrp_log">${translate('Logarithmic')}
<br>
...
`;
  ...
  weektoweek.prepareHtml = function weektoweekPrepareHtml (weekstoshow) {
    ...
    var legend = `<table>
<tr>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Sunday')}</td>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Monday')}</td>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Tuesday')}</td>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Wednesday')}</td>
</tr>
<tr>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Thursday')}</td>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Friday')}</td>
<td><svg width="16" height="16"><g><circle fill="${dayColors[colorIdx++]}" r="40"></circle></g></svg></td><td>${translate('Saturday')}</td>
</tr>
</table>`;
  ...
  weektoweek.report = function report_weektoweek (datastorage, sorteddaystoshow, options) {
    ...
    weekstoshow.forEach(function eachWeek (week) {
      const sgvData = [];
      const weekStart = moment(`${week[0]} 00:00:00`);
    ...
    function drawChart (week, sgvData, options) {
      ...
      charts = d3.select(`#weektoweekchart-${week[0]}-${week[week.length - 1]}`).html(
        `<b>${report_plugins.utils.localeDate(week[0])}-${report_plugins.utils.localeDate(week[week.length - 1])}</b><br>`
      ).append('svg');
