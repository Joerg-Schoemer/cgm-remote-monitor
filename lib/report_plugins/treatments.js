...
treatments.html = function html(client) {
  ...
  return '<h2>' + translate('Treatments') + '</h2>'
    ...
    + '     <input type="number" step="any" min="0" id="rped_absorptionTime" placeholder="' + translate('Duration in minutes') + '" />'
    + '  </field>'
    + '  <field>'
...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    table.append($('<tr>').append($('<td>').attr('colspan', 14).addClass('next_day').append(report_plugins.utils.localeDate(day))));
    ...
    for (const treatment of treatments) {
      ...
      table.append($('<tr>')
        ...
        .attr('day', day)
        .append(editCell)
        .append($('<td>').addClass('time').append(new Date(treatment.created_at).toLocaleTimeString().replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, '$1$3')))
